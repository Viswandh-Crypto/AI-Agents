...
class OutlineValidationChecker(Agent):
    def __init__(self):
        super().__init__(
            ...
            model=MODEL,
            description="Validation that the outline is usable.",
            instruction="""
...
class BlogPostValidationChecker(Agent):
    def __init__(self):
        super().__init__(
            ...
            instruction="""
Check `blog_post` for: intro, clear sections matching the outline, conclusion, and techincal clarity.
If passes, respond "ok". Else respond "retry" with specific fixes.
""",
            output_key="validation_result", 
        )
...
3.) End with 3 alternative titles and 2 twwet-length hooks.
